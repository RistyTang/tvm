...
void matmul_fp32(float* A, float* B, float* C, int32_t core_mask) {
  if ((GetCoreNum(core_mask) > 0) && (GetLogicCoreId(core_mask, DNUM) >= 0)) {
    int32_t ax0_0;
    for (ax0_0 = (GetLogicCoreId(core_mask, DNUM) * (4 / GetCoreNum(core_mask))); ax0_0 < ((GetLogicCoreId(core_mask, DNUM) == (GetCoreNum(core_mask) - 1)) ? 4 : ((GetLogicCoreId(core_mask, DNUM) + 1) * (4 / GetCoreNum(core_mask)))); ++ax0_0) {
      float* A_global_l2 = (float*)(276889600 + DNUM * 16777216);
      int32_t ax1_0;
      for (ax1_0 = 0; ax1_0 < 4; ++ax1_0) {
        int32_t ax2_0;
        for (ax2_0 = 0; ax2_0 < 4; ++ax2_0) {
          int32_t cse_v2 = (ax2_0 * 32);
          load_row_major_tile((&(A[0])), (&(A_global_l2[0])), (ax0_0 * 32), cse_v2, 32, 32, 128, 4);
          int32_t cse_v1 = (ax1_0 * 32);
          load_row_major_tile((&(B[0])), (&(A_global_l2[16384])), cse_v2, cse_v1, 32, 32, 128, 4);
          int32_t ax0_1;
          for (ax0_1 = 0; ax0_1 < 32; ++ax0_1) {
            ...
            for (ax1_1 = 0; ax1_1 < 32; ++ax1_1) {
              ...
              for (ax2_1 = 0; ax2_1 < 32; ++ax2_1) {
                ...
                int32_t cse_v4 = ((cse_v3 + cse_v1) + ax1_1);
                if ((cse_v2 + ax2_1) == 0) {
                  C[cse_v4] = 0.000000e+00f;
                }
                C[cse_v4] = (C[cse_v4] + (A_global_l2[((cse_v3 + cse_v2) + ax2_1)] * A_global_l2[(((((ax2_0 * 4096) + (ax2_1 * 128)) + cse_v1) + ax1_1) + 16384)]));
              }
            }
          }
        }
