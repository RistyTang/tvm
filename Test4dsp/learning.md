...
- **L2**：每核当前可用窗口为 `0x000F0000` 字节，对应物理地址
  `0x00810000 ~ 0x008FFFFF`；在生成 C 中按
  `l2_base_core0 + DNUM * l2_core_stride` 绑定，默认
  `l2_base_core0 = 0x10810000`、`l2_core_stride = 0x01000000`。
...
- **当前 `tvm.tirx.build` 输出已脱去 TVM FFI wrapper 且接通多核派发（A.7 + A.8 已落地）**：c6678 host PrimFunc 现在以 `void matmul_fp32(float* A, float* B, float* C, int32_t core_mask)` 形态出码，`ForKind.PARALLEL` 的目标形态已经切换为 `if (GetCoreNum > 0 && GetLogicCoreId(core_mask, DNUM) >= 0) { ... }` 多核分块循环 + 末尾 `C6678E_SyncN(...)`，BSP 端可以 `extern void matmul_fp32(...)` 直接链接。
- **A.9（C6678DMALower）+ A.10 step1（dlight schedule 端 cache_read/annotate）联合 PR 已落地（✅，2026-05-24）**：
  ...
  - A.9 已拆为两段 pass（见 §4.8.3 第 3 条）：① `C6678DMALower` 在 `ConvertBlocksToOpaque` **之前**（`PlanAndUpdateBufferAllocationLocation` 之后）执行，把带 `c6678.dma_load` 注解的 staging block 替换为 `Evaluate(call_extern("load_row_major_tile", src.data, dst.data, row0, col0, rows, cols, src_ld, sizeof_elem))`；② `C6678AnnotateL2Alloc` 在 `StorageRewrite` **之后**、`LowerTVMBuiltin` **之前**执行，给 `scope=="global.l2"` 的 `AllocBuffer` 通过 `tirx.decl_buffer(scope="global", ...)` 重建 `Buffer.data` 的 `PointerType.storage_scope = "global"`，然后第二轮 `ir_transform` 把 body 中所有 `BufferLoad/BufferStore` 的 `buffer` 字段一并替换为新 Buffer，再用 `stmt_functor.substitute` 把 PrimExpr 里的 data Var 引用替换；同时给 `AllocBuffer` 留下 `disable_lower_builtin = True` 注解作为 `LowerTVMBuiltin` 端的兜底。当前目标形态不再是普通栈数组，而是由 codegen 按 per-core L2 基址打印成指针。
...
[default_s_tir_pipeline] —— 挂载点 ① schedule 起点
  ...
  • C6678MulticoreLower         ★ A.8 ✅ PARALLEL→GetCoreNum/GetLogicCoreId(core_mask, DNUM)/C6678E_SyncN
...
| `int32_t GetLogicCoreId(int32_t core_mask, int32_t core_id)` | A.8 `C6678MulticoreLower` + `CodeGenC6678` | 逻辑核 ID 获取接口，生成 C 中直接保留 `GetLogicCoreId(core_mask, DNUM)` |
| `void C6678E_SyncN(int32_t core_num, int32_t core_id)` | A.8 `C6678MulticoreLower` | 同步 N 核（替代 IR 侧不可用的 barrier） |
| `void load_row_major_tile(void* src_base, void* dst, int32_t row0, int32_t col0, int32_t rows, int32_t cols, int32_t src_ld, int32_t elem_bytes)` | A.9 `C6678DMALower` | DMA 搬运一块 tile 到 L2 staging |
...
| A.8 | `C6678MulticoreLower`（路径 2） | `python/tvm/tirx/transform/c6678_multicore_lower.py` | ✅（识别 `ForKind.PARALLEL` → 改写成 `if (GetCoreNum > 0 && GetLogicCoreId(core_mask, DNUM) >= 0) { for ... }` + 末尾 `C6678E_SyncN`，并给入口追加 `int32_t core_mask` 形参；与 A.7 链在 `_c6678_pre_packed_api_passes`） |
...
| `core_id = GetLogicCoreId(core_mask, DNUM)`、`C6678E_SyncN(core_num, core_id)` | A.8 `C6678MulticoreLower` 把 `parallel`/`thread_binding="coreIdx"` 转换为这套 BSP 调用 | ✅（IR 中直接构造 `call_extern("GetLogicCoreId", core_mask, DNUM)`，并通过 `CodeGenC6678` 保证 `DNUM` 以裸宏名输出） |
...
| A.8 | `C6678MulticoreLower` ✅ | **实际落点**：`python/tvm/tirx/transform/c6678_multicore_lower.py`（pure Python `prim_func_pass`）+ 在 `_c6678_pre_packed_api_passes()` 紧跟 A.7 之后。识别 `ForKind.PARALLEL` 循环（顶层 `parallel(extent)`），用 `tirx.stmt_functor.ir_transform` 把它替换为 `if (GetCoreNum > 0 && GetLogicCoreId(core_mask, DNUM) >= 0) { for v=core_id*chunk; v < (core_id==N-1?extent:(core_id+1)*chunk); ++v) {<body>} }` + `Evaluate(call_extern("C6678E_SyncN", core_num, core_id))`，并在 `func.params` 末尾追加 `core_mask: int32`，落 `c6678.multicore_lowered=True` 标记保证幂等。关键约束是 `tirx` 无法自然表达裸宏 `DNUM`，因此额外用了最小 `CodeGenC6678` 定制来直接打印该符号。 | A.2、A.7 |
...
2. **A.8** ✅：`C6678MulticoreLower` 接管 `parallel` 注解。
   实测产物：[`generated_c6678_matmul_via_build.c`](file:///home/tangqingyun/tvm/Test4dsp/tests/generated_c6678_matmul_via_build.c) 入口签名已经扩为 `void matmul_fp32(float* A, float* B, float* C, int32_t core_mask)`；外层 `T.parallel(4)` 已被替换为 `if ((GetCoreNum(core_mask) > 0) && (GetLogicCoreId(core_mask, DNUM) >= 0)) { for (...) {...} }`，函数尾部出 `C6678E_SyncN(GetCoreNum(core_mask), GetLogicCoreId(core_mask, DNUM))`。
...
3. **A.9 + A.10（必须联合 PR）**：`C6678DMALower` 接管 `cache_read`/`storage_plan` 中 L2 / SMC scope；`dlight.c6678.Matmul` 同步加 `cache_read("global.l2") + compute_at(k_outer)` 并对 staging block `sch.annotate(...)`。
   ...
   - A.9 pass 端：拆为两段。① `C6678DMALower` 在 `ConvertBlocksToOpaque` **之前**（`PlanAndUpdateBufferAllocationLocation` 之后）执行，要求 `SBlockRealize.iter_values` 仍然存在；匹配 `For(ax0)/For(ax1)/SBlockRealize(staging_block)` 三层结构，取 `staging_realize.iter_values` 把 `ax0/ax1` 替换为 0 得到 `row0/col0`、外层 `For.extent` 即 `rows/cols`、`block.reads[0].buffer.shape[-1]` 即 `src_ld`，把整个三层 For/SBlockRealize 替换为 `Evaluate(call_extern("", "load_row_major_tile", src.data, dst.data, row0, col0, rows, cols, src_ld, IntImm("int32", elem_bytes)))`。② `C6678AnnotateL2Alloc` 在 `StorageRewrite` **之后**、`LowerTVMBuiltin` **之前**执行（`StorageRewrite` 会丢弃 `AllocBuffer` 上除 `kVolatile` 外的所有 annotation，所以必须在它之后跑）；该 pass 找到所有 `buffer.scope() == "global.l2"` 的 `AllocBuffer`，重建 `Buffer.data` Var（`PointerType.storage_scope = "global"`）并通过 `stmt_functor.substitute` 把 body 中所有引用一并替换，并追加 `disable_lower_builtin = True` 兜底注解；后续 `CodeGenC6678` 再把带特定 annotation 的 alloc 打印成 per-core L2 指针。
   ...
   产物（目标）：`generated_c6678_matmul_via_build.c` 出现 `float* A_global_l2 = (float*)(0x10810000 + DNUM * 0x01000000);` + `load_row_major_tile(A, A_global_l2, ax0_0*32, ax2_0*32, 32, 32, 128, 4);` + 对应 B 同形态调用，与 `examples.md` 的 BSP 风格一致。
   **当前实现状态（2026-05-24，✅ 已落地）**：
   ...
   - **Follow-up 已闭环（2026-05-24）**：`StorageRewrite` 把 `A_global.l2` 与 `B_global.l2` 合并到同一块 alloc + 用 elem_offset=16384 区分。原本 `call_extern("load_row_major_tile", ..., dst.data, ...)` 是不透明节点，`StorageRewrite` 不会改写其参数列表，导致 B 的 DMA 第二参数仍是头部。**采纳方案 (a)**：在 [`C6678DMALower::_build_dma_call`](file:///home/tangqingyun/tvm/python/tvm/tirx/transform/c6678_dma_lower.py) 用 `src_buf.access_ptr("r") / dst_buf.access_ptr("w")` 替代裸 `.data`，`tvm_access_ptr` builtin 会被 [`storage_rewrite.cc:1601-1625`](file:///home/tangqingyun/tvm/src/tirx/transform/storage_rewrite.cc#L1601-L1625) 专门识别并合成正确偏移参数；后续 `LowerIntrin` 将 `tvm_access_ptr` 降级为 `address_of(BufferLoad)`，C codegen 出 `&A_global_l2[16384]` 形态，而 alloc 本身的最终目标形态已改为 per-core L2 指针。修复后目标出码：
     ```c
    load_row_major_tile((&(A[0])), (&(A_global_l2[0])),     (ax0_0 * 32), cse_v1, 32, 32, 128, 4);
    load_row_major_tile((&(B[0])), (&(A_global_l2[16384])),  cse_v1, cse_v2, 32, 32, 128, 4);
     ```
...
void softmax_fp32(float* A, float* Out, int32_t core_mask) {
  if ((GetCoreNum(core_mask) > 0) && (GetLogicCoreId(core_mask, DNUM) >= 0)) {
    int32_t ax0;
    for (ax0 = (GetLogicCoreId(core_mask, DNUM) * (8 / GetCoreNum(core_mask))); ...) {
...
- **A.8 已落地**：`C6678MulticoreLower`（pure Python pass）识别 `ForKind.PARALLEL` 顶层并行循环，在 IR 层用 `tirx.stmt_functor.ir_transform` 把它降级为 `if (GetCoreNum > 0 && GetLogicCoreId(core_mask, DNUM) >= 0) { for ... }` + 末尾 `C6678E_SyncN`，并在入口形参末尾追加 `int32_t core_mask`；与 A.7 同处 `_c6678_pre_packed_api_passes()` 紧贴 `MakePackedAPI` 之前；
...
1. **L2 容量估算修正**：`C6678Features.static_alloc_l2_bytes` 不再只估算两个 32×32 tile（8192B），而是按当前 matmul schedule 最终可能生成的 A/B 两个读 buffer staging 全量估算；128×128×128 fp32 matmul 现在为 131072B。这样 dispatcher 的 L2 gate 更保守，不会用过小估算误放候选模板，也不再把“容量估算值”误写成“最终 C 的栈数组长度”。
